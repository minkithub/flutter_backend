...
import 'package:kakao_sample_profile/src/component/text_edit_widget.dart';
...
class Profile extends GetView<ProfileController> {
  ...
  Widget _backgroundImageWidget() {
    return controller.myProfile.value.backgroundUrl == null
        ? Container()
        : Image.network(controller.myProfile.value.backgroundUrl,
            fit: BoxFit.cover);
  }
      ...
      child: Obx(() => Column(
            children: [
              _partProfileInfo(controller.myProfile.value.name, () async {
                ...
                if (value != null) {
                  controller.updateName(value); //value값을 업데이트
                }
              }),
              _partProfileInfo(controller.myProfile.value.discription,
                ...
                if (value != null) {
                  controller.updateDiscription(value); //value값을 업데이트
                }
              }),
            ],
          )),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      resizeToAvoidBottomPadding: false, //리사이징이 안되므로 키보드 겹침 현상 예방.
